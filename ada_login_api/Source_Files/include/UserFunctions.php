<?php
//ChromePhp::log('page: UserFunctions.php');
...
class UserFunctions {
	...
	public function register($params,$conn) {
		...
		if($emailTaken || $nameTaken) {
			...
			$response["success"] = 0;
			$response["message"] = $message . $nameTaken . $emailTaken;
			return json_encode($response);
		} else {
	...
	public function update_home_posts($params, $conn) {
		$response = array();
		$home_posts = array();
		$user_id = $params["user_id"];
		$latestPost = $params["timestamp"];
		$exists = $this->check_if_user_exists_by_id($user_id,$conn);
		if(!$exists) {
			$response["success"] = 0;
			$response["message"] = "User with given id does not exist.";
		...
		try {
			...
			$stmt = $conn->prepare('SELECT post.*,user.username FROM post,user WHERE post.poster_id IN ('.$friends_ids_r["friends_ids"].') AND user.user_id=post.poster_id AND post.timestamp > '.$latestPost.' ORDER BY timestamp DESC');
			//$stmt = $conn->prepare('SELECT post.*,user.username FROM post,user WHERE post.poster_id IN ('.$friends_ids_r["friends_ids"].') AND post.poster_id=user.user_id ORDER BY timestamp DESC');			
			if($stmt->execute()){
				$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
				$response["success"] = 1;
				$response["message"] = "Home posts updated successfully.";
				$response["home_posts"] = $result;
			} else {
				$response["success"] = 0;
				$response["message"] =	"Home posts could not be updated.";
			}
			return json_encode($response);
		} catch (PDOException $e) {
			ChromePhp::log($e->getMessage());//echo $e->getMessage();
	...
	public function get_user_posts($params, $conn) {
		...
		try {
			...
			$stmt = $conn->prepare('SELECT post.*,user.username FROM post,user WHERE post.poster_id=:user_id AND user.user_id=post.poster_id order by timestamp desc');
			$stmt->bindValue(':user_id',$user_id);
			if($stmt->execute()) {
				$response["success"] = 1;
	...
	public function update_user_posts($params, $conn) {
		$response = array();
		try {
			$latestPost = $params['timestamp'];
			$user_id = $params["user_id"];
			...
			$stmt = $conn->prepare('SELECT post.*,user.username FROM post,user WHERE post.poster_id=:user_id AND user.user_id=post.poster_id AND post.timestamp > :latestPost order by timestamp desc');
			$stmt->bindValue(':user_id',$user_id);
			$stmt->bindValue(':latestPost',$latestPost);
			if($stmt->execute()) {
				$response["success"] = 1;
				$response["message"] = "Posts retreived successfully.";
				$response["posts"] = $stmt->fetchAll(PDO::FETCH_ASSOC);
